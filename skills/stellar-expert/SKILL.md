---
name: stellar-expert
description: Look up on-chain Stellar network data via the stellar.expert Explorer API — accounts, assets, ledgers, liquidity pools, markets, Soroban contracts, asset prices/holders, network stats, and the well-known-address directory. Use when the user asks about a Stellar account/address (G...), a Stellar asset or token, XLM, a Soroban contract (C...), a liquidity pool (L...), a ledger, Stellar network activity/stats, or anything referencing stellar.expert. Works on mainnet (public) and testnet.
---
...
- **Per-account payment/operation history** — not exposed by stellar.expert. For that, use
  Horizon directly: `https://horizon.stellar.org/accounts/{id}/payments`.
